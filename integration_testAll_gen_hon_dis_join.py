...
if __name__ == '__main__':
    ...
    time.sleep(BEAT)
    assert(len(bchain.chain) == 2) # different forks length 2
    assert(bchain.validate_chain())
    logging.info("{}: Current blockchain: {}".format(bchain.whoami, str(bchain)))
    time.sleep(BEAT)

    logging.info("==== d. The honest node mines another block and sends it to the generator node, who accepts it, and the dishonest node, which rejects it. ====")
    if bchain.whoami == "honest":
        bchain.generate()
    ...
    logging.info("==== e. The joiner node requests a copy of the blockchain from all the other nodes and accepts the longest valid. ====")
    if bchain.whoami == "joiner":
        bchain.chain=[]
        bchain.broadcast_request_chain()

    # Wait for joiner to come online
    time.sleep(BEAT)
    assert(bchain.validate_chain())
    logging.info("{}: Current blockchain: {}".format(bchain.whoami, str(bchain)))
    time.sleep(BEAT)
