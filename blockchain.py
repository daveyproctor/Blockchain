...
def serverDispatch(server_socket):
    while 1:
        ...
        if server_socket in ready_to_read:
            ...
            network.add_client_socket(s)
            print("Client (%s, %s) connected" % addr)
        else:
            print("Odd behavior in server")
      
def clientTryConnect(servers):
    printedSet = set()
    while len(servers) > 0:
        for peer in servers:
            # I'll be the client
            ...
            s.settimeout(2)
            try:
                s.connect((HOST, network.get_node_port(peer)))
                network.add_client_socket(s)
                
                print("Connected to {}".format(peer))
                network.add_node("localhost", network.get_node_port(peer))
                servers.remove(peer)
                break
...
if __name__ == '__main__':
    ...
    global network
    network = Network( difficulty=18 )

    # track who's serving over which ports
    for i, node in enumerate(allNodes):
        network.set_node_port(node, PORT_MIN+i)

    # start server per node
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server_socket.bind((HOST, network.get_node_port(whoami)))
    server_socket.listen(5)
    x = threading.Thread(target=serverDispatch, args=(server_socket,))
    x.start()

    # Try connect to peers
    servers = set(peer1 for peer1, peer2 in itertools.combinations(allNodes, 2) if peer2==whoami)
    y = threading.Thread(target=clientTryConnect, args=(servers,))
    y.start()
